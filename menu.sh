...
show_header() {
  ...
  if [[ -f "$PRO_CONFIG" ]]; then
    if [[ -n "$TELEGRAM_BOT_TOKEN" ]]; then
      pro_badge="${GREEN}aktif${NC} ${DIM}(limit device/kuota + Telegram)${NC}"
    else
      pro_badge="${GREEN}aktif${NC} ${DIM}(limit device/kuota)${NC}"
    fi
  fi
...
main_menu() {
  ...
  menu_line 14 "Bot Telegram & Limit Akun (Pro)"
  echo -e "${CYAN}$BOX_MID${NC}"
  ...
  echo -e "${CYAN}$BOX_BOT${NC}"
  echo ""
  echo -ne "  ${WHITE}Pilih menu [0-14]${NC}: "
  read -r choice
  ...
  case "$choice" in
    ...
    14) bash $SCRIPT_DIR/menu/telegram.sh ;;
    0) clear; exit 0 ;;
